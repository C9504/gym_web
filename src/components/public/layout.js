                    ...
                    <Navbar.Collapse id="navbarResponsive">
                        <Nav className="ms-auto my-2 my-lg-0">
                            ...
                            {!keycloak.authenticated && (
                                <Nav.Item>
                                    <Button variant="primary" size="sm" onClick={() => keycloak.register({ redirectUri: window.location.origin })}>Unirme</Button>
                                </Nav.Item>
                            )}
                            {keycloak.authenticated && (
                                <NavDropdown title={<span><i className="bi bi-person-circle text-success" /> {keycloak?.profile?.firstName}</span>} id="basic-nav-dropdown-settings">
                                    <NavDropdown.Item as={Link} to={"/profile"}><i className="bi bi-x-lg text-success"></i> Perfil</NavDropdown.Item>
